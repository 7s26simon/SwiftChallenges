...
class Library {
// key is ORW, book id is 1985, author is George Orwell
    var catalogue = ["ORW":Book("1985", "George Orwell"),
                     "RAY":Book("Fahrenheit 451", "Ray Bradbury")]
    
    // empty dictionary, key is String, value is person object (person who checked out book)
    ...
    func searchByTitle(_ title:String) -> String {
        ...
        // pass in title of a book, not book ID, actual title
        // if book title is not in catalogue, return not in catalogue
        // if in catalogue and available, return available
        // if exists and checked out, show who has checked it out
        // hint: check both dictionaries
        
        for (bookId, _) in catalogue {
           ...
           // check if available
            if checkedOutBooks[bookId] == nil {
                ...
                return "Not available, book checked out by" + (person?.name)!
                    }
